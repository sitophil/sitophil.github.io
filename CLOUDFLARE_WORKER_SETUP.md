# راهنمای راه‌اندازی ورکر کلودفلر برای فرم تماس
...
۲. کد موجود در فایل را کاملاً پاک کنید و کد زیر را جایگزین آن کنید:
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // این متغیرها در مرحله بعد در پنل تنظیم می‌شوند
      const botToken = env.TELEGRAM_BOT_TOKEN;
      const chatId = env.TELEGRAM_CHAT_ID;

      const text = `🚀 <b>پیام جدید از سایت:</b>\n\n👤 <b>نام:</b> ${name}\n📞 <b>تلفن:</b> ${phone}\n💼 <b>حوزه کاری:</b> ${work}`;
...
۳. روی دکمه **Save and Deploy** کلیک کنید.

## مرحله ۵: تنظیم متغیرهای محیطی (Variables)
۱. به صفحه تنظیمات ورکر خود برگردید (در پنل کلودفلر).
۲. به لبه (Tab) **Settings** و سپس بخش **Variables** بروید.
۳. در قسمت **Environment Variables**، دو متغیر زیر را اضافه کنید:
   - نام: `TELEGRAM_BOT_TOKEN` | مقدار: (توکنی که از BotFather گرفتید)
   - نام: `TELEGRAM_CHAT_ID` | مقدار: (آیدی عددی خودتان)
۴. روی **Save and deploy** کلیک کنید.

## مرحله ۶: اتصال وب‌سایت به ورکر (مهم‌ترین مرحله)
۱. در همان صفحه اول ورکر در پنل کلودفلر، آدرس URL ورکر خود را کپی کنید (چیزی شبیه `https://sitophil-contact-form.username.workers.dev`). **حتماً دقت کنید که آدرس با https شروع شود.**
۲. فایل `index.html` سایت خود را باز کنید.
۳. به انتهای فایل بروید و اسکریپت مربوط به فرم را پیدا کنید.
۴. عبارت `'YOUR_CLOUDFLARE_WORKER_URL'` را پیدا کرده و آدرس کپی شده را دقیقاً جایگزین آن کنید (داخل کوتیشن‌ها).
۵. فایل را ذخیره و در هاست خود آپلود کنید.

**نکته:** اگر این مرحله را انجام ندهید، دکمه ارسال کار نخواهد کرد و خطای "تنظیم ورکر" را دریافت می‌کنید.

**تبریک! فرم تماس شما اکنون به تلگرام متصل است.**
